...
filtered_df = autoscrape_df[autoscrape_df["Category"].isin(["Criminal and Civil Actions", "State Enforcement Agencies"])]
...
filtered_df = autoscrape_df[autoscrape_df["Category"] == "Criminal and Civil Actions"]
...
# Apply the function to the "Agency" column in autoscrape_df
autoscrape_df["State"] = autoscrape_df["Agency"].apply(assign_state)

# Calculate the number of enforcement actions per state
state_counts = autoscrape_df["State"].value_counts().reset_index()
...
# Convert to GeoJSON and load as a dictionary
geojson_data = json.loads(state_shp_merged.to_json())

# Use Altair to create a choropleth map
choropleth = alt.Chart(alt.Data(values=geojson_data['features'])).mark_geoshape().encode(
    color=alt.Color("properties.Count:Q", title="Number of Enforcement Actions", scale=alt.Scale(scheme="blues")),
    tooltip=["properties.NAME:N", "properties.Count:Q"]
).project(
    type="albersUsa"
).properties(
...
# Apply the function to the "Agency" column in autoscrape_df
autoscrape_df["District"] = autoscrape_df["Agency"].apply(assign_district)

# Calculate the number of enforcement actions per district
district_counts = autoscrape_df["District"].value_counts().reset_index()
...
# Convert to GeoJSON and load as a dictionary
geojson_data = json.loads(district_shp_merged.to_json())

# Use Altair to create a choropleth map
choropleth = alt.Chart(alt.Data(values=geojson_data['features'])).mark_geoshape().encode(
    color=alt.Color("properties.Count:Q", title="Number of Enforcement Actions", scale=alt.Scale(scheme="greens")),
    tooltip=["properties.NAME:N", "properties.Count:Q"]
).project(
    type="albersUsa"
).properties(
...
# Latest autoscrape_df contains data from January 2021
# Data in district_counts is the aggregated number of enforcements in autoscrape_df by district 
...
choropleth = alt.Chart(alt.Data(values=geojson_data['features'])).mark_geoshape().encode(
    color=alt.Color("properties.Ratio:Q", title="Enforcement Action Ratio", scale=alt.Scale(scheme="reds")),
    tooltip=["properties.judicial_d:N", "properties.Ratio:Q", "properties.Count:Q", "properties.Total Population:Q"]
).project(
    type="albersUsa"
).properties(
